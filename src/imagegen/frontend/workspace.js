...
export function setupWorkspace(root = document) {
  ...
  const csrfToken = documentRoot
    .querySelector('meta[name="csrf-token"]')
    ?.getAttribute("content");
  const pageChecksum = documentRoot
  ...
  function loadJsonArray(selector) {
    const value = readJsonScript(selector, { root, fallback: [] });
  ...
  sourceWorkflow = setupSourceImages(documentRoot, {
