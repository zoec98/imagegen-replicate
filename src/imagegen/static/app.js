...
(function() {
	...
	(() => {
		...
		function populatePaletteEditor(selectedPalette = "", selectedFragment = "") {
			if (!paletteEditorPalette || !paletteEditorFragment) return;
			...
			const palette = paletteData.find((item) => item.name === paletteEditorPalette.value) || paletteData[0] || null;
		...
		async function refreshPaletteData(selectedPalette = "", selectedFragment = "") {
			...
			const response = await fetch(form.dataset.apiPalettesUrl, { credentials: "same-origin" });
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Palette refresh failed.");
		...
		async function loadEditorFragment() {
			if (!paletteEditorPalette?.value || !paletteEditorFragment?.value) {
				if (paletteEditorContent) paletteEditorContent.value = "";
				return;
			}
			const response = await fetch(paletteUrl(`/${encoded(paletteEditorPalette.value)}/fragments/${encoded(paletteEditorFragment.value)}`), { credentials: "same-origin" });
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Palette fragment could not be loaded.");
			if (paletteEditorName) paletteEditorName.value = "";
			if (paletteEditorContent) paletteEditorContent.value = data.fragment?.content || "";
		}
		async function writePaletteFragment(method, url, body) {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Palette edit failed.");
			return data;
		}
		async function createPaletteFragment() {
			...
			const data = await writePaletteFragment("POST", paletteUrl(`/${encoded(paletteName)}/fragments`), {
				name,
				content
			});
		...
		async function updatePaletteFragment() {
			...
			const content = paletteEditorContent?.value || "";
			const data = await writePaletteFragment("PUT", paletteUrl(`/${encoded(paletteName)}/fragments/${encoded(fragmentName)}`), { content });
		...
		async function deletePaletteFragment() {
			const paletteName = paletteEditorPalette?.value || "";
			const fragmentName = paletteEditorFragment?.value || "";
			const data = await writePaletteFragment("DELETE", paletteUrl(`/${encoded(paletteName)}/fragments/${encoded(fragmentName)}`), {});
		...
		async function reportImmichThumbnailError(thumbnailUrl) {
			...
			try {
				const response = await fetch(thumbnailUrl, { credentials: "same-origin" });
				if (response.ok) return;
				setUploadStatus((await readApiJson(response, "Immich thumbnail could not be loaded.")).error || "Immich thumbnail could not be loaded.", "error");
			} catch (error) {
		...
		async function loadImmichPage(page) {
			...
			try {
				renderImmichAssets(await readApiJson(await fetch(url.toString(), { credentials: "same-origin" }), "Immich gallery could not be loaded."));
			} catch (error) {
				setImmichPageLabel(`Page ${immichCurrentPage}`);
				throw error;
			} finally {
		...
		async function saveMaskEditorData() {
			...
			try {
				...
				const data = await response.json();
				if (!response.ok) throw new Error(data.error || "Mask could not be saved.");
				closeMaskEditor();
				await refreshGallery();
		...
		async function checkAppFreshness() {
			...
			const response = await fetch(form.dataset.apiAppVersionUrl, { credentials: "same-origin" });
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "App version check failed.");
		...
		async function loadGalleryMetadata(figure) {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Image metadata could not be loaded.");
		...
		async function metadataForInfo(figure) {
			...
			const response = await fetch(figure.dataset.metadataUrl, { credentials: "same-origin" });
		...
		async function refreshTrash() {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Trash refresh failed.");
		...
		async function restoreTrashImage(figure) {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || `Could not restore ${filename}.`);
		...
		async function emptyTrash() {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Trash could not be emptied.");
			if (Object.prototype.hasOwnProperty.call(data, "trash_count")) setTrashCount(data.trash_count);
			await refreshTrash();
		...
		async function refreshGallery() {
			...
			disarmGalleryDelete();
			const response = await fetch(form.dataset.apiImagesUrl, { credentials: "same-origin" });
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || "Gallery refresh failed.");
			if (Object.prototype.hasOwnProperty.call(data, "trash_count")) setTrashCount(data.trash_count);
		...
		async function pollGeneration(statusUrl) {
			...
			try {
				...
				const data = await response.json();
				if (!response.ok) throw new Error(data.error || "Generation status request failed.");
		...
		paletteEditorToggle?.addEventListener("click", () => {
			...
			if (nextOpen) {
				populatePaletteEditor(paletteEditorPalette?.value || "", paletteEditorFragment?.value || "");
