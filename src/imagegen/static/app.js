...
(function() {
		...
		async function importUploadFiles(files, emptyMessage) {
			...
			try {
				for (const [index, file] of accepted.entries()) {
					...
					try {
						const data = await postForm(overlay?.dataset.apiUploadUrl, formData, "Image file could not be uploaded.");
						imported.push(data?.image?.filename || uploadFileLabel(file));
					} catch (error) {
						failed.push(`${uploadFileLabel(file)}: ${error.message || "upload failed"}`);
					}
				}
				if (imported.length) await refreshGallery();
				if (failed.length) {
					const uploaded = `${imported.length} image${imported.length === 1 ? "" : "s"} uploaded`;
					setStatus(`${imported.length ? uploaded : "No images uploaded"}; failed: ${failed.join(", ")}.`, "error");
				} else if (imported.length === 1) setStatus(`${imported[0]} imported.`, "success");
				else setStatus(`${imported.length} images uploaded.`, "success");
			} catch (error) {
	...
	function pointerPosition(event, rect, width, height) {
		if (!rect || rect.width <= 0 || rect.height <= 0 || !width || !height) return null;
		return {
	...
	function cropRectangle(start, end, width, height) {
		if (!start || !end || !width || !height) return null;
	...
	function isValidCropSelection(selection, minimumSize = 10) {
		return Boolean(selection?.width >= minimumSize && selection?.height >= minimumSize);
	}
	function paintMask(maskData, width, height, position, brushSize, falloff) {
		if (!maskData || !width || !height || !position) return;
	...
	function setupImageEditor(root = document, services = {}) {
		...
		function pointerPosition$1(event) {
			return pointerPosition(event, maskCanvas?.getBoundingClientRect(), maskCanvas?.width, maskCanvas?.height);
		}
		function paintAt(position) {
			if (!position || !maskCanvas || !maskData) return;
			paintMask(maskData, maskCanvas.width, maskCanvas.height, position, brushSize, brushFalloff);
			redrawOverlay();
			updateBlurButton();
		}
		function isValidCropSelection$1() {
			return isValidCropSelection(cropSelection);
		}
		function updateCropControls() {
			if (cropButton) cropButton.disabled = !isValidCropSelection$1();
		}
		...
		function startCrop(event) {
			...
			const position = pointerPosition$1(event);
			if (!position) return;
			event.preventDefault();
			cropStart = position;
			cropSelection = null;
		...
		function continueCrop(event) {
			...
			if (!position) return;
			event.preventDefault();
			cropSelection = cropRectangle(cropStart, position, maskCanvas?.width, maskCanvas?.height);
			updateCropControls();
			redrawOverlay();
		}
		...
		async function crop() {
			...
			if (!isValidCropSelection$1()) return;
		...
		function startPainting(event) {
			...
			isPainting = true;
			maskCanvas.setPointerCapture?.(event.pointerId);
			paintAt(pointerPosition$1(event));
		}
