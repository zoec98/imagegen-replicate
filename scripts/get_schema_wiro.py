...
def request_json(
    ...
    *,
    ...
    try:
        ...
        raise SchemaError(f"HTTP {error.code} from Wiro Tool Detail.") from error
    except URLError as error:
        raise SchemaError(f"could not reach Wiro Tool Detail: {error.reason}") from error
    except OSError as error:
        raise SchemaError(f"could not read Wiro Tool Detail response: {error}") from error
...
def print_report(model_ref: str, tool: Mapping[str, Any]) -> None:
    ...
    if categories:
        print(f"- Categories: {', '.join(f'`{_safe_text(item)}`' for item in categories)}")
        print(f"- Text-to-image: {'yes' if _has_category(categories, 'text-to-image') else 'no'}")
        print(f"- Image-to-image: {'yes' if _has_category(categories, 'image-to-image') else 'no'}")
...
def print_parameters(raw: Any) -> None:
    ...
    if not entries:
        ...
        print()
        return
    print("| Name | Type | Required | Default | Choices | Bounds | File limit | Description |")
    print("| --- | --- | --- | --- | --- | --- | ---: | --- |")
    for entry in entries:
        name = first_value(entry, "name", "field", "fieldname", "parameter", "key", "id")
        if not name:
            name = "(unnamed)"
        type_name = first_value(entry, "type", "datatype", "inputtype", "dataType") or "unknown"
        required = first_value(entry, "required", "isrequired", "mandatory")
        default = first_value(entry, "default", "defaultValue", "value")
        choices = choice_values(entry)
        minimum = first_value(entry, "minimum", "min", "minValue")
        maximum = first_value(entry, "maximum", "max", "maxValue")
        ...
        if file_limit is None and str(type_name).lower() in {"file", "image", "file[]", "image[]"}:
            file_limit = maximum
        bounds = ""
        if minimum is not None or maximum is not None:
            bounds = f"{minimum if minimum is not None else ''}..{maximum if maximum is not None else ''}"
        description = first_value(entry, "description", "help", "label") or ""
        cells = (name, type_name, _required_text(required), default, ", ".join(map(str, choices)), bounds, file_limit, description)
    ...
    if source_limits:
        ...
        for entry in source_limits:
            name = first_value(entry, "name", "field", "fieldname", "parameter", "key", "id") or "(unnamed)"
            limit = first_value(
                entry,
                "maxFiles",
                "max_files",
            ...
            print(f"  - `{name}`: maximum `{limit}` source file(s)" if limit is not None else f"  - `{name}`: file input")
        print()
...
def choice_values(entry: Mapping[str, Any]) -> list[Any]:
    ...
    for item in value:
        if isinstance(item, dict):
            result.append(first_value(item, "value", "id", "name", "label") or item)
        else:
            result.append(item)
    return result
...
def _looks_like_source(entry: Mapping[str, Any]) -> bool:
    name = _safe_text(first_value(entry, "name", "field", "fieldname", "parameter", "key", "id"))
    type_name = _safe_text(first_value(entry, "type", "datatype", "inputtype", "dataType"))
    return "image" in name.lower() or type_name.lower() in {"file", "image", "file[]", "image[]"}
