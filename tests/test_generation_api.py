...
def test_api_generate_rejects_wrong_provider_model_alias(app_config, app_factory):
    ...
    assert response.status_code == 400
    assert response.json == {
        "error": "Unknown model: flux-flex. Expected one of: bria-fibo, ernie-image, ernie-image-turbo, flux-2, flux-2-pro, flux-2-realism, gpt-image-2, gpt-image-25-sunburst, gpt-image15, grok, hidream-dev, hidream-fast, hidream-full, krea-2-large, krea-2-medium, krea-2-turbo, nano-banana-2, seedream, seedream45, seedream5, seedream5-pro, zit."
    }
